    ...
    <v-container ><!-- fluid grid-list-md -->
      <v-layout row wrap>
        <v-flex xs12 sm12 md12 class="category_history">
          <p><router-link :to="{ path: '/' }">Inicio</router-link> > <a :href="`/category/${$route.params.subCategory}/`">{{ MaysPrimera(slugToString($route.params.subCategory).toLowerCase()) }}</a> > {{ MaysPrimera(slugToString($route.params.id).toLowerCase()) }}</p>
        </v-flex>
      </v-layout>
    </v-container>
                  ...
                  <v-list>
                    <v-list-group>
                      ...
                      <v-container class="scroll-y max-h400" id="scroll-target">
                        <v-layout column>
                          <v-list-tile v-for="(item, index) in subCategories" :key="index">
                            <v-list-tile-content>
                              <v-list-tile-title class="list-sub-cate"><a :href="`/category/${$route.params.subCategory}/${$route.params.id }/${stringToSlug(item.category)}`" class="category_subtitle">{{ MaysPrimera(item.category.toLowerCase()) }}</a></v-list-tile-title>
                            </v-list-tile-content>
                          </v-list-tile>
                        </v-layout>
                      </v-container>
...
export default {
  async asyncData (context) {
    ...
    return  axios.get(`${context.env.apiTag}/subcategories/slug/${context.params.id}`)
    .then((response) => {
      return {
  ...
  methods:{
    getSubCategories(item){
      ...
      axios({
        method:'GET',
        url:`${this.apiTag}/categoriesLevelTwo/name/${this.slugToString(item)}`,
      })
